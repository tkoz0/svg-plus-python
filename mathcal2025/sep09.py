...
image = svgimage((0,0),(5,7),150,210)
...
C = c+(0,-r-R)
a = atan(r/3)
P = c+vec.polarr(r,pi+a)
u = vec.polarr(0.3,a)
uu = u.rotated(-90)

image += group([
    rect(C,C+(R,2*r+R),attrs=attrs(fill='#7cf')),
    circle(r,c),
    circle(R,C),
    circle(0.1,c,attrs=attrs(fill='black')),
    line(c,P),
    polyline([P+u,P+u+uu,P+uu]),
    line(C,P,attrs=attrs(stroke='red'))
],style_line1)

image += group([
    text('3',1.3,3.3,attrs(fill='red')),
    text('x',3.7,4.8)
],style_font1)
