...
image = svgimage((0,0),(5,6),250,300)
...
image += group([
    polygon([C,D,vec.midpoint(A,C)]),
    rect(vec.midpoint(A,B),vec.midpoint(B,C))
],style_line1.fill('#7cf').stroke('none'))
...
image += group([
    rect(A,C),
    line(A,C),
    line(D,vec.midpoint(A,C)),
    polyline([vec.midpoint(A,B),vec.midpoint(A,C),vec.midpoint(B,C)]),
    polyline([C+(-0.4,0),C+(-0.4,-0.4),C+(0,-0.4)]),
    tick1(A,vec.midpoint(A,B)),
    tick1(vec.midpoint(A,B),B),
    tick1(B,vec.midpoint(B,C)),
    tick1(vec.midpoint(A,B),vec.midpoint(A,C)),
    tick1(vec.midpoint(A,C),vec.midpoint(B,C)),
    *tick2(C,D),
    *tick2(A,D)
],style_line1)
...
image += group([
    text('4',A+(-0.6,2)),
    text('Find the blue area',A+(-0.3,-0.5))
],style_font1)
